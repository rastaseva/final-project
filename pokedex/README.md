...
Для запуска приложения, необходимо установить директорию 'pokedex', как основную, а затем уже, соответственно, использовать команду npm start.
После компиляции приложения, нажмите на кнопку 'Home', чтобы начать отображение покемонов.
